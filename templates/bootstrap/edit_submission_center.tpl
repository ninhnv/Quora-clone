...
<form style="margin-left: 20px;" action="" method="post" id="thisform" enctype="multipart/form-data" >
	...
	*}
	...
	{if $SubmitSummary_Allow_Edit eq 1}
		...
		<br />
	{/if}
	<strong style="">Chuyên mục: </strong>
	<span style="display:none" class="field-description">{#PLIGG_Visual_Submit2_CatInstruct#}</span>
	<br />
	{if $Multiple_Categories}
		{section name=thecat loop=$submit_cat_array}
			{$submit_cat_array[thecat].spacercount|repeat_count:'&nbsp;&nbsp;&nbsp;&nbsp;'}
				 <input style="display:none" type="checkbox" class="form-control" name="category[]" value="{$submit_cat_array[thecat].auto_id}" {if $submit_cat_array[thecat].auto_id == $submit_category  || in_array($submit_cat_array[thecat].auto_id,$submit_additional_cats)}checked{/if}> {$submit_cat_array[thecat].name}<br />							
		{/section}
	{else}
		<select style="" class="form-control" {if $Multiple_Categories}name="category[]" multiple size=10{else}name="category"{/if}>
			{section name=thecat loop=$submit_cat_array}
	...
	<br />
	<input type="submit" value="{#PLIGG_Visual_Submit2_Continue#}" class="btn btn-default" />
</form>
