...
<div class="submit_page">
	<legend class="headline-custom" style="font-size: 15px;">Posts</legend>
	...
	<form class="form-horizontal" action="{$URL_submit}" method="post" name="thisform" id="thisform" enctype="multipart/form-data" onsubmit="return checkForm()">
		<div class="col-md-6 submit_step_2_left">
				...
				<div class="controls select-category{if $Multiple_Categories} multi-select-category{/if}">
					...
					{else}
						<select id="category" class="form-control category" tabindex="2" name="category" onchange="if ($('#category option:selected').val()>0) $('#lp-category').text($('#category option:selected').text()); else $('#lp-category').text('');">
							<option value="1">{#PLIGG_Visual_Submit2_CatInstructSelect#}</option>
							{section name=thecat loop=$submit_cat_array}
			...
			{if $enable_tags}
				<div class="control-group">
					...
					<div class="controls">
						...
						<p class="help-inline">Example: web, funny, gif</p>
					</div>
				</div>
			{/if}
			...
			{if $SubmitSummary_Allow_Edit eq 1}
				<div class="control-group">
					...
					<div class="controls">
						...
						{/if *}
						<textarea style="width:100%" name="summarytext" rows="4" maxlength="{$maxSummaryLength}" id="summarytext" class="col-md-4" WRAP="SOFT">{$submit_summary}</textarea>
						<br />
						<p class="help-inline">
			...
			<div class="control-group">
				...
				<div class="controls">
					<textarea name="bodytext" tabindex="15" rows="8" class="form-control bodytext col-md-4" id="bodytext" maxlength="{$maxStoryLength}" WRAP="SOFT">{if $submit_url_description}{$submit_url_description}{/if}{$submit_content}</textarea>
					<br />
				...
				<div class="storyfooter">
					<div class="story-tools-left">
						...
						<span id="ls_comments_url-{$link_shakebox_index}">
							<i class="fa fa-comment"></i> <span id="linksummaryDiscuss"><a class="comments">Bình luận</a>&nbsp;</span>
						</span> 
						<i class="fa fa-star"></i> <span id="linksummarySaveLink"><a id="add" class="favorite" >Lưu tin</a></span>&nbsp;
