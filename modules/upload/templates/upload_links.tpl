...
{if sizeof($images)}
	...
	{foreach from=$images item=image}
		...
		{if strpos($image.file_name,'http')===0}
			...
			<img style="max-width:100%;padding-left: 80px;" src='{$image.file_name}'/> 
		{else}
			{*<a href='{$my_pligg_base}{$upload_directory}/{$image.file_name}'>{$image.file_name}</a>*}
			<img style="max-width:100%;padding-left: 80px;" src='http://linkhayvn.com/modules/upload/attachments/{$image.file_name}'/>
		{/if}
	    {/if}
	{/foreach}
{$upload_post_format}
