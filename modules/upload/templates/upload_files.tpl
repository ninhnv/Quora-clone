...
<h2 >Đăng ảnh</h2>
<span >({$upload_extensions} {#PLIGG_Upload_Extensions_Allowed#})</span><br /><br />
...
>
	<fieldset style="border:1px solid #eee;padding:10px;margin-bottom:10px;font-weight:bold;width:450px;">
		...
		<input type='hidden' name='id' value='{$submit_id}'>
    	    ...
    	    {/if}
    	    {if strstr($upload_external,'url')}
		{#PLIGG_Upload_Link#}: <input type='text' name='upload_urls[]' id='url_{$number}' placeholder="Nhập link hình ảnh ở đây" !onchange='submitUploadForm(this.form)'>
    	    {/if}
